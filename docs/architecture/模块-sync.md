...
2. **比对阶段**：构建目标文件集合，按 `SyncMode` 计算差异集
   ...
   - FULL：全量覆盖，目标已存在的文件也重新同步（目标多余文件删除，删除失败计入失败明细）
...
- **短事务**：仅"加载任务 + 创建执行记录"在事务内提交；扫描/下载/上传等网络 I/O 在事务外执行，避免长事务持锁数小时、中途失败整体回滚
- **并发防重**：同一任务并发触发（定时调度与手动触发重叠）时直接忽略，防止重复执行导致双删/双移源文件
- **分页保护**：目录扫描翻页上限 1000 页（约 10 万文件），防止异常场景死循环
