...
- [ ] T040 [US2] 在 `src/main/java/top/lldwb/alistmediasync/service/TranscodeService.java` 中实现转码引擎（核心）：
  ...
  - **单文件转码**：`doTranscode(TranscodeCandidate candidate)` 内部流程：
    ...
    3. 构建 JAVE2 转码参数：
       - **MP3 目标**（仅音频）：`AudioAttributes`（codec=libmp3lame、bitrate=128000、channels=2、samplingRate=44100），省略 `VideoAttributes`
       - **MP4 目标**（视频+音频）：`VideoAttributes`（codec=libx264、保持原始分辨率/帧率）+ `AudioAttributes`（codec=aac、bitrate=128000、channels=2、samplingRate=44100）
       - **FLV 目标**（视频+音频）：`VideoAttributes`（codec=flv、保持原始分辨率/帧率）+ `AudioAttributes`（codec=libmp3lame、bitrate=128000、channels=2、samplingRate=44100）
...
| 002 任务 | 对应 001 阶段 | 说明 |
|---------|-------------|------|
| 002 T004/T005（TempFileManager） | 001 阶段 4 T034 | 临时文件创建/重命名/删除/权限 |
| 002 T006（DiskSpaceChecker） | 001 阶段 4 T035 | 磁盘空间预估检查 |
| 002 T003（TempSuffixValidator） | 001 阶段 4 T036 | 启动时后缀配置校验 |
| 002 T009（CleanupService 增强） | 001 阶段 7 T053 | 启动清理 + 手动清理接口 |
| 002 T008（并发上限 Semaphore） | 001 阶段 4 T040 | TranscodeService 信号量控制 |
