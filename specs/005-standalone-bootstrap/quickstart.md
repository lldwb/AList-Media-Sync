...
# 4. 关闭应用后检查配置文件，确认密码值未被修改
cat config/application.yaml | grep password
# 预期输出：password: "test123"（仍为明文，未被修改）

# 5. 再次启动应用，确认每次启动均输出加密日志
sh start.sh
```

### 预期结果

- 每次启动日志均输出："检测到明文密码，已自动加密为 BCrypt 格式"
- 配置文件中的 `password` 值始终保持为用户写入的明文 `"test123"`，不会被修改
- 使用 `test123` 可通过 HTTP Basic 认证登录管理界面
- 加密后的 BCrypt 值仅存在于内存中，不持久化到任何文件
