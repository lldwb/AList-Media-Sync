...
`app.auth.password` 配置值的状态转换。

```
[配置文件中的 app.auth.password 值]
         │
         ▼
   ┌──────────────────┐
...
**边界情况**：
- 配置文件被编辑器锁定（Windows）→ 写回失败 → 日志错误 → 应用继续启动（使用内存中的加密值，但下次启动仍会检测到明文）
- 环境变量 `APP_AUTH_PASSWORD` 覆盖 → 加密逻辑同样对覆盖后的值生效
