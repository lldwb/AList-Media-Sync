...
同步影响报告
============================================================
版本变更：1.10.0 → 1.11.0（MINOR — 新增原则 XII「Git 提交规范」，固化提交策略治理）
修改原则：
  - IV. 中文优先 — 提交信息条款补充"遵循原则 XII 的提交策略与信息结构"引用
新增原则：
  - XII. Git 提交规范（不可协商）— 固化提交前检查、单一职责拆分、显式 git add、提交信息结构四项治理规则
    * 提交前 MUST 通过 git status + git diff 分析改动涉及的功能模块
    * 多模块改动 MUST 按模块拆分为多次独立提交（业务逻辑/配置/文档/测试各归一次）
    * 单模块或单文件改动 MUST 合并为一次提交，不可强制拆分
    * git add MUST 显式指定文件，禁止 git add -A / git add .
    * 提交信息 MUST 使用中文，采用简明摘要 + 必要详细说明结构，遵循 Conventional Commits 与仓库现有风格
新增部分：
  - 开发工作流质量门禁新增 Git 提交规范合规检查
  - AGENTS.md 合规检查清单新增第 12 项
移除部分：无
模板同步状态：
  ✅ plan-template.md — 章程检查为占位符，无需修改
  ✅ spec-template.md — 无需修改
  ✅ tasks-template.md — 无需修改
  ✅ AGENTS.md（根级） — 版本引用同步至 1.11.0；AI 工作指令新增第 12 条；合规检查清单新增第 12 项
...
- Git 提交信息 MUST 使用中文，遵循 Conventional Commits 格式（`feat: 添加媒体文件扫描功能`），并遵循原则 XII 的提交策略与信息结构
...
所有本地 Git 提交 MUST 遵循以下四项治理规则，确保提交历史清晰、可追溯、可回滚：

#### 12.1 提交前检查（不可省略）

- 每次提交前 MUST 先通过 `git status` 与 `git diff`（含 `--staged`）查看所有已修改文件及其具体改动内容
- MUST 分析这些改动所涉及的功能模块（如业务逻辑、配置项、文档、测试、依赖等），为下一步提交策略决策提供依据
- MUST NOT 在未查看改动内容的情况下直接执行 `git add` 与 `git commit`

#### 12.2 单一职责提交策略（不可协商）

根据单一职责原则决定提交策略：

- **多模块拆分**：当改动涉及多个相互独立的功能模块时，MUST 按模块拆分为多次独立提交，每次提交仅包含同一功能模块的改动。模块划分示例：
  - 业务逻辑变更（Service/Controller/Repository 等代码改动）归为一次提交
  - 配置项调整（`application.yaml`、`pom.xml`、Dockerfile 等）归为一次提交
  - 文档更新（`docs/`、`README.md`、`CHANGELOG.md` 等）归为一次提交
  - 测试改动（`src/test/` 下的单元测试、集成测试）归为一次提交
- **单模块合并**：当所有改动均属于同一功能模块，或仅有单个文件被修改时，MUST 合并为一次提交，不可强制拆分
- **拆分边界**：拆分粒度以"功能模块"为准，不以"文件数量"为准；同一功能模块的多个文件改动 MUST 合并在同一次提交中

#### 12.3 显式文件暂存（不可协商）

- 每次提交时 MUST 使用 `git add` 显式指定本次提交所涉及的具体文件路径
- MUST NOT 使用 `git add -A`、`git add .` 或 `git add -u` 等批量暂存命令，以防误纳入无关文件或敏感文件（如 `.env`、本地配置、密钥文件）
- 暂存前 SHOULD 复核 `git status` 输出，确认暂存清单与本次提交的模块边界一致

#### 12.4 提交信息结构（不可协商）

- 提交信息 MUST 使用简体中文撰写（遵循原则 IV）
- MUST 准确说明本次提交的具体改动内容与目的，不得仅写"更新代码"等模糊描述
- MUST 采用「简明摘要 + 必要详细说明」的结构：
  - **摘要行**：遵循 Conventional Commits 格式（`<type>: <描述>`），类型包括 `feat`（新功能）、`fix`（缺陷修复）、`docs`（文档）、`style`（格式）、`refactor`（重构）、`test`（测试）、`chore`（构建/工具）、`perf`（性能）等；摘要行不超过 72 字符
  - **详细说明**：空一行后撰写，说明改动的背景、目的与关键变更点；可使用要点列表；如有关联 Issue 或 spec，MUST 在末尾引用
- MUST 遵循仓库现有的提交信息风格（参见 `git log` 历史记录的格式惯例）

**理由**：Git 提交历史是项目演进的"时间轴档案"，是代码审查、问题回溯、版本回滚的核心依据。批量暂存与模糊提交信息会让"哪次改动引入了问题"变得不可追溯；混合多模块改动的单次提交则让回滚变得"全有或全无"——为了撤销一处配置改动不得不连带回滚业务代码。将"提交前检查 → 单一职责拆分 → 显式暂存 → 结构化信息"固化为不可协商流程，是确保提交历史始终可读、可追溯、可精准回滚的唯一可靠方式。
...
- 代码审查 MUST 验证 Git 提交是否遵循单一职责拆分、显式 `git add`、结构化中文提交信息（原则 XII）
...
**版本**：1.11.0 | **批准日期**：2026-06-19 | **最近修订**：2026-07-25
