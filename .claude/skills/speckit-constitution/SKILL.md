...
此命令自身的工作仅限于更新项目章程，并将章程驱动的变更传播到本命令中识别的相关制品。

- 将用户输入的每一部分分类为章程内容或单独的**非治理意图**。
- 如果输入包含功能实现、代码生成、重构、构建或部署请求，你**不得**执行它们。将它们提取为延迟意图。
- 你**不得**创建、修改或删除应用程序源文件、功能路由、组件、测试、部署文件，或与章程工作流及其必需传播无关的其他工件。
- 如果不确定某个指令是否为章程内容，在做出更改前要求澄清。
- 完成章程更新后，为每个延迟的意图包含一个"后续操作"部分。列出原始意图并建议适当的后续 Spec Kit 命令，例如 `speckit-specify`，但不调用它。
- 如果没有非治理意图，则省略"后续操作"部分。
...
4. 一致性传播检查清单（将先前的检查清单转换为主动验证）：
   ...
   - 读取为你代理安装的每个 Spec Kit 命令文件（包括此文件）— 命名为 `speckit.*` 或 `speckit-*`（点或连字符取决于代理），或布局为 `speckit-<name>/SKILL.md`（用于基于 skills 的集成），例如在 `.github/agents/`、`.github/skills/`、`.claude/skills/` 或你代理的等效命令目录中 — 验证在需要通用指导时没有过时的引用（仅 CLAUDE 或其他代理特定名称）残留。
